function [vp,vd,q,bp,bpr,def,rr,totaltime,avgtime] = solve_benchmark_cuda(z,m,b,pdf,para) 
...
coder.gpu.kernel()
for is = 1:ns  % to calculate the value of default
    coder.gpu.constantMemory(z); 
    coder.gpu.constantMemory(m); 
    y = exp(z(is))*m(is) ;
    for ib = 1:nb
        ...
        for i = 1: nb
            ...
            if q(is,i) >= 0.45 
            c = y - (eta+(1-eta)*coup)*b(ib) + q(is,i)*(b(i)*m(is) - (1-eta)*b(ib)) ;
              if c <= 0
                  W(i, ib, is) = - Inf; 
              else
        ...
        [vpnew(is,ib), bp(is,ib)]  = max( W(:, ib, is) ) ;
    end
end


coder.gpu.kernel()
for is = 1:ns % to incorporate taste shocks on debt choice
    for ib = 1:nb
        sumExp = 0;
        sumExpQ = 0;
        ...
        for i = 1:nb
            ...
            coder.gpu.constantMemory(q);
            temp = W(i, ib, is) - vpnew(is, ib) - cv_bp;
            if temp > 0
...
pdef = 1./(1 + exp((vpnew - vdnew)/sigg_defp)) ;

rrnew = pdf*((1-mu)*rr + mu*probDcre./b')/(1 + rstar) ; 
qnew = pdf*(pdef.*rrnew + (1-pdef).*qnew)/(1 + rstar) ;
