...
bool checkGridSize(const dim3* in) {
    double prod = double(in->x) * double(in->y) * double(in->z);
    return in->x <= MAX_BLOCKS_PER_GRID_DIM_X && in->y <= MAX_BLOCKS_PER_GRID_DIM_YZ &&
           in->z <= MAX_BLOCKS_PER_GRID_DIM_YZ && prod > 0 && prod <= INT32_MAX;
}
...
void getLaunchParametersImpl(unsigned long long numIterations,
                             dim3* grid,
                             dim3* block,
                             unsigned int maxBlocks,
                             unsigned int maxThreadsPerBlock) {

    if (numIterations <= static_cast<unsigned long long>(maxThreadsPerBlock) * maxBlocks) {
        // Tune to avoid trivial imbalanced kernel configuration.
        unsigned int numThreads = maxThreadsPerBlock;
        unsigned long long numBlocks = divCeil(numIterations, numThreads);
...
void mwGetLaunchParameters(unsigned long long numIterations,
                           ...
                           dim3* block,
                           unsigned int maxBlocks) {
    getLaunchParametersImpl(numIterations, grid, block, maxBlocks, 256);
}
...
void mwGetLaunchParametersReduction(unsigned long long numIterations,
                                    ...
                                    dim3* block,
                                    unsigned int maxBlocks) {
    getLaunchParametersImpl(numIterations, grid, block, maxBlocks, MAX_THREADS_PER_BLOCK);
}
...
void mwApplyLaunchParameters(unsigned long long numIterations,
    ...
    if (numIterations == 0) {
        ...
        *outblock = inblock;
        ...
        mwGetLaunchParameters(numIterations, outgrid, outblock, INT32_MAX);
    }
}
