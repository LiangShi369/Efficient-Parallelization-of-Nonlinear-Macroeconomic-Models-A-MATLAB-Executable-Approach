...
#include "solver_divide_mexutil.h"
...
static emlrtRSInfo emlrtRSI = {
    23,              /* lineNo */
    "solver_divide", /* fcnName */
    "D:\\BaiduSyncdisk\\working papers\\Parallel computation with "
    "Mex\\code_github\\Code sovereignDefault\\solver_divide.m" /* pathName */
};
...
static emlrtBCInfo emlrtBCI = {
    ...
    "bmonoNonRec", /* fName */
    "D:\\BaiduSyncdisk\\working papers\\Parallel computation with "
    "Mex\\code_github\\Code sovereignDefault\\solver_divide.m", /* pName */
    0                                                           /* checkKind */
};
...
void solver_divide(solver_divideStackData *SD, const emlrtStack *sp,
                   ...
                   real_T vp[250000], boolean_T def[250000], real_T *totaltime,
                   real_T *avgtime)
{
  ...
  int32_T k;
  int32_T nb0;
  int32_T solver_divide_numThreads;
  boolean_T exitg1;
  st.prev = sp;
  st.tls = sp->tls;
  b_st.prev = &st;
  ...
  solver_divide_numThreads = emlrtAllocRegionTLSs(
      sp->tls, omp_in_parallel(), omp_get_max_threads(), omp_get_num_procs());
#pragma omp parallel for num_threads(solver_divide_numThreads)
  ...
  while ((diff > 1.0E-7) && (its < 1000)) {
    ...
    for (is = 0; is < 625; is++) {
      ...
      for (k = 0; k < 400; k++) {
        coffset = is + 625 * k;
        w[k] = SD->f0.w[coffset];
        evp[k] = SD->f0.evp[coffset];
        b_def[k] = def[coffset];
      }
      st.site = &e_emlrtRSI;
      bmonoNonRec(&st, b, w, evp, vd1[is], b_def, varargin_1, dv, dv1);
      for (k = 0; k < 400; k++) {
        coffset = is + 625 * k;
        SD->f0.vp1[coffset] = varargin_1[k];
        bp[coffset] = dv[k];
        SD->f0.def1[coffset] = (dv1[k] != 0.0);
      }
      if (*emlrtBreakCheckR2012bFlagVar != 0) {
        emlrtBreakCheckR2012b((emlrtConstCTX)sp);
      }
    ...
    for (k = 0; k <= 249998; k += 2) {
      ...
      dv3[0] = muDoubleScalarAbs(dv4[0]);
      dv3[1] = muDoubleScalarAbs(dv4[1]);
      r = _mm_loadu_pd(&dv3[0]);
      _mm_storeu_pd(&SD->f0.w[k], r);
    }
    if (!muDoubleScalarIsNaN(SD->f0.w[0])) {
    ...
    for (k = 0; k <= 622; k += 2) {
      ...
      dv3[0] = muDoubleScalarAbs(dv5[0]);
      dv3[1] = muDoubleScalarAbs(dv5[1]);
      r = _mm_loadu_pd(&dv3[0]);
      _mm_storeu_pd(&x[k], r);
    }
...
/* End of code generation (solver_divide.c) */
