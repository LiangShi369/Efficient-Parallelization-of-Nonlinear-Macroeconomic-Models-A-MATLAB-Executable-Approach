...
if heteroagentoptions.do_GE==1
    % Find GE prices
    minoptions = optimset('TolX',heteroagentoptions.toleranceGEprices,'TolFun',heteroagentoptions.toleranceGEcondns,'MaxFunEvals',heteroagentoptions.maxiter);
    p_eqm = fminsearch(@(p) model_subfuns_parfor(p, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions),GEPrices0,minoptions);
    [~,V,Policy,StatDist] = model_subfuns_parfor(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions); % Recompute model objects at GE prices
elseif heteroagentoptions.do_GE==0
    p_eqm = GEPrices0;
    [~,V,Policy,StatDist] = model_subfuns_parfor(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions);
end

Outputs_parfor = compute_targets(a_grid, z_grid, pi_z, Policy, StatDist, Params, p_eqm) ;
totaltime_parfor = toc(smctime)
make_table(ResFolder,Outputs_parfor,Params) ;
...
if heteroagentoptions.do_GE==1
    % Find GE prices
    minoptions = optimset('TolX',heteroagentoptions.toleranceGEprices,'TolFun',heteroagentoptions.toleranceGEcondns,'MaxFunEvals',heteroagentoptions.maxiter);
    p_eqm = fminsearch(@(p) model_subfuns_vec(p, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions),GEPrices0,minoptions);
    [~,V,Policy,StatDist] = model_subfuns_vec(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions); % Recompute model objects at GE prices
elseif heteroagentoptions.do_GE==0
    p_eqm = GEPrices0;
    [~,V,Policy,StatDist] = model_subfuns_vec(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions);
end

Outputs_vec = compute_targets(a_grid, z_grid, pi_z, Policy, StatDist, Params, p_eqm) ;
totaltime_vec = toc(smctime)
make_table(ResFolder,Outputs_vec,Params) ;
...
if heteroagentoptions.do_GE==1
    % Find GE prices
    minoptions = optimset('TolX',heteroagentoptions.toleranceGEprices,'TolFun',heteroagentoptions.toleranceGEcondns,'MaxFunEvals',heteroagentoptions.maxiter);
    p_eqm = fminsearch(@(p) model_subfuns_gpuarray(p, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions),GEPrices0,minoptions);
    [~,V,Policy,StatDist] = model_subfuns_gpuarray(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions); % Recompute model objects at GE prices
elseif heteroagentoptions.do_GE==0
    p_eqm = GEPrices0;
    [~,V,Policy,StatDist] = model_subfuns_gpuarray(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions);
end

Outputs_gpuarray = compute_targets(a_grid, z_grid, pi_z, Policy, StatDist, Params, p_eqm) ;
totaltime_gpuarray = toc(smctime)
make_table(ResFolder,Outputs_gpuarray,Params) ;
...
cfg = coder.config('mex');
cfg.GenerateReport = true;
codegen -config cfg fun_VFI_parfor2 -args {zeros(1,2), a_grid, z_grid, pi_z, Params, vfoptions} -o fun_VFI_parfor_mex
...
if heteroagentoptions.do_GE==1
    % Find GE prices
    minoptions = optimset('TolX',heteroagentoptions.toleranceGEprices,'TolFun',heteroagentoptions.toleranceGEcondns,'MaxFunEvals',heteroagentoptions.maxiter);
    p_eqm = fminsearch(@(p) model_subfuns_mex(p, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions),GEPrices0,minoptions);
    [~,V,Policy,StatDist] = model_subfuns_mex(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions); % Recompute model objects at GE prices
elseif heteroagentoptions.do_GE==0
    p_eqm = GEPrices0;
    [~,V,Policy,StatDist] = model_subfuns_mex(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions);
end

Outputs_parfor = compute_targets(a_grid, z_grid, pi_z, Policy, StatDist, Params, p_eqm) ;
totaltime_mex_serial = toc(smctime)
...
if heteroagentoptions.do_GE==1
    % Find GE prices
    minoptions = optimset('TolX',heteroagentoptions.toleranceGEprices,'TolFun',heteroagentoptions.toleranceGEcondns,'MaxFunEvals',heteroagentoptions.maxiter);
    p_eqm = fminsearch(@(p) model_subfuns_parfor_mex(p, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions),GEPrices0,minoptions);
    [~,V,Policy,StatDist] = model_subfuns_parfor_mex(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions); % Recompute model objects at GE prices
elseif heteroagentoptions.do_GE==0
    p_eqm = GEPrices0;
    [~,V,Policy,StatDist] = model_subfuns_parfor_mex(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions, simoptions, heteroagentoptions);
end
